...
def run_train(
    ...
    for itr in progress_bar:
        ...
        optimizer.zero_grad()
        # scheduler调整的是优化器的学习率
        # optimizer是根据梯度，计算反向传播的梯度
        scheduler.step()
        print("ccc")
    ...
    with torch.no_grad():
        for itr in progress_bar:
            ...
            if cfg.val_amp is True:
                ...
                val_outputs = sliding_window_inference(val_inputs, cfg.roi_size, cfg.sw_batch_size, model)
            # cal metric
            # tta指的是test time augmentation测试时数据增强
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--seed", type=int, default=888, help="seed")
